...
<!-- 여기는 자신의 기능을 적을것-->
<h6 class=subTitle> 교육센터 통합 운영관리 시스템 - 출결대장 관리 - 출결대장 조회</h6>

<h3 class=mainTitle>출결대장 상세조회 </h3>

 <fieldset>
      <form action="attandanceRecordUpdate.jsp">
...
<%} %>
         ...
         <%
            ...
            for (int i = 0; i < totalPerson; i++){
            	String record = arc.getRecord(arr1.get(i).memberID, pc);
            	System.out.println("배열 : " + record);
            	String [] records = record.split("/");
            	System.out.println("배열 길이 : " + records.length);
            	%>
            	<tr>
            	<td><input type="text" name="id<%=i %>" value="<%=arr1.get(i).memberID %>" style='width:100px'></td>
            	<% for(int j = 0; j < totalDate; j++){ %>
            	<td><input type="text" name="<%=i%>/<%=j%>" value="<%=records[j]%>" style='width:20px'></td>
      ...
      <input class="myButton" type="submit" value="수정">
